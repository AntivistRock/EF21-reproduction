...
class EF21(torch.optim.Optimizer):
    ...
    def _top_k_compression(self, tensor, k_ratio):
        k = int(k_ratio * tensor.numel())
        ...
        for group in self.param_groups:
            for p in group["params"]:
                if dist.get_rank() != 0:
                    ...
                    c = self._top_k_compression(p.grad.data - self.state[p]['g'], 1)  # line 5
                ...
                if dist.get_rank() == 0:
                    ...
                    p.data -= group['lr'] * self.state[p]['g']  # not sure we have to type .data there
